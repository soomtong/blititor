...
uic = {
								...
								switch(this.className) {
									...
									case 'film':									
										...
										if(!$('#email').val()){
											...
											$('#email').focus();
											return false;	
										} else if($('#email').val().search(/(\S+)@(\S+)\.(\S+)/) == -1){
										...
										if(!$('#hp').val()){
											...
											$('#hp').focus();
											return false;	
										} else if($('#hp').val().search(/(\S+)-(\S+)-(\S+)/) == -1){
											alert("휴대폰번호가 잘못 입력되었습니다.\n01x-xxx-xxxx 형식으로 입력해 주십시요.");
											$('#hp').focus();
											return false;	
										}
								
										document.thisForm.action = "http://welltag.com/2015/osscon/reg_confirm.asp";
										document.thisForm.target = "ifr_hidden";
										document.thisForm.submit();
										 										
										$('#change_name').text($('#name').val());
	...
	init: function() {
		var own = this, c = this.container = Qsa('#wrap')[0];
		if(!c) return;
		this.snb.init();
		this.layer.init();
...
function chk_regi(){
	...
	if(!$('#chk_email').val()){
		...
		$('#chk_email').focus();
		return false;	
	} else if($('#chk_email').val().search(/(\S+)@(\S+)\.(\S+)/) == -1){
		alert("이메일이 잘못 입력되었습니다.\n정확하게 입력해 주십시요.");
		$('#chk_email').focus();
		return false;
	}
	
	document.checkForm.action = "http://welltag.com/2015/osscon/check.asp";
	document.checkForm.target = "ifr_hidden";
