						...
						Qsa('div.hgroup div.btn_over a', function(i, a) {
							...
							a.onclick = function() {
								switch(this.className) {
									...
									case 'films':
										uic.layer.show('layer_films', function() {
											if(!this.interaction) return;
											this.interaction.openData.setfilms(this.interaction.selectedItem, true);
										});
										break;
								}
								return false;
			...
			Qsa('div.view_layer', function(i, div) {
				...
				switch(id) 
				{
					...
					case 'layer_films':
						...
						div.interaction.selectItemAt(0, true);
						div.interaction.openData.setfilms = function(item, onOff) {
							if(!item) return;
							item.innerHTML = onOff? item.getAttribute('ifrcode'): '';
						};
						//div.interaction.openData.setfilms(div.interaction.selectedItem, true);

						div.rollBtns = Qsa('span.roll_btn a', function(i, a) {
							a.index = i;
							a.onclick = function() {
								div.interaction.openData.setfilms(div.interaction.selectedItem, false);
		...
		show: function(id, callback, hideCallback) {
			...
			if(!PopupLayer.dimmer.onclick) {
				...
				PopupLayer.dimmer.onclick = function(e) {
					own.hide(own._sid_);
					if(own._sid_ == 'layer_films') {
						var interaction = own.elements['layer_films'].interaction;
						interaction.openData.setfilms(interaction.selectedItem, false);
					}
					return false;
				};
			}
